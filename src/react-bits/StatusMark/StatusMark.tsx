...
export type StatusMarkStatus = keyof typeof TEXT
...
export default function StatusMark({
  ...
  useEffect(() => {
    ...
    void animate(mode, solid ? 1 : 0, MORPH)
    void animate(arc, targetArc, UI)
    if (indeterminate) {
      const t0 = travel.get()
      void animate(travel, [t0, t0 - C], { duration: spinDuration / 1000, ease: 'linear', repeat: Infinity })
      return
    }
    const unit = determinate ? C : P
    const to = Math.floor(travel.get() / unit) * unit
    void animate(travel, to, UI).then(() => {
