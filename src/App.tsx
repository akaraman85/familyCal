    ...
    <div className="assistant-body">
      ...
      {turns.map((turn, index) => <div className="planner-turn" key={turn.id}>
        ...
        <div className="ai-message"><div className="assistant-symbol small"><Sparkles size={14}/></div><div>
          ...
          {turn.proposal.events.length > 0 && <div className="proposal-events"><p className="proposal-event-count">{turn.proposal.events.length} event{turn.proposal.events.length === 1 ? '' : 's'} ready to add</p>{turn.proposal.events.map((event, eventIndex) => <div className="parsed-event" key={`${event.startAt}-${event.title}-${eventIndex}`}><div className="parsed-date"><b>{proposalDatePart(event, turn.timezone, 'day')}</b><span>{proposalDatePart(event, turn.timezone, 'month')}</span></div><div><b>{event.title}</b><span><Clock3 size={13}/>{proposalTime(event, turn.timezone)} · {event.calendar}</span>{event.location && <span><MapPin size={13}/>{event.location}</span>}</div></div>)}</div>}
